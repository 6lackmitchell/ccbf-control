...
from bicycle.settings.intersection.jerk_control import *
...
file_addendum = delimiter + '..' + delimiter + 'bicycle' + delimiter + 'datastore' + delimiter + 'intersection' + delimiter + 'jerk' + delimiter
filepath = os.path.dirname(os.path.abspath(__file__)) + file_addendum
...
ax_cont_a.plot(t[1:ii], alpha_max * np.ones(t[1:ii].shape), linewidth=lwidth+1, color='k')
ax_cont_a.plot(t[1:ii], -alpha_max * np.ones(t[1:ii].shape), linewidth=lwidth+1, color='k')
...
for aa in range(nAgents):
    ...
    ax_cont_a.plot(t[:ii], sols[:ii, aa, 1], label='a_{}'.format(aa), linewidth=lwidth,
                   color=colors[color_idx[aa, 0]])
    ax_cont_a.plot(t[:ii], sols_nom[:ii, aa, 1], label='a_{}^0'.format(aa), linewidth=lwidth,
                   color=colors[color_idx[aa, 1]], dashes=dash)
ax_cont_a.set(ylabel='a',#ylabel=r'$\omega$',
              ylim=[-alpha_max - 0.5, alpha_max + 0.5],
...
ax_cont_b.plot(t[1:ii], jr_max * np.ones(t[1:ii].shape), linewidth=lwidth+1, color='k')
ax_cont_b.plot(t[1:ii], -jr_max * np.ones(t[1:ii].shape), linewidth=lwidth+1, color='k')
for aa in range(nAgents):
    ...
    #                color=colors[color_idx[aa, 1]], dashes=dash)
    ax_cont_b.plot(t[:ii], sols[:ii, aa, 0], label='a_{}'.format(aa), linewidth=lwidth,
                   color=colors[color_idx[aa, 0]])
    ax_cont_b.plot(t[:ii], sols_nom[:ii, aa, 0], label='a_{}^0'.format(aa), linewidth=lwidth,
                   color=colors[color_idx[aa, 1]], dashes=dash)
ax_cont_b.set(ylabel='a',#ylabel=r'$a_r$',
              ylim=[-jr_max - 1, jr_max + 1])
...
if video:
    ...
    # Create animation
    ani = animation.FuncAnimation(fig=fig_map, func=animate, frames=int(ii / 1), interval=10, repeat=False)
