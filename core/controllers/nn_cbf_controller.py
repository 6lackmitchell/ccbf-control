...
class NnCbfController(CbfQpController):
    ...
    def _generate_cbf_condition(self,
        ...
        self.LsB = -k * B * cbf.dhdx_value @ np.diag(np.concatenate([np.diagonal(sigma([])), np.diagonal(sigma([]))]))
        ...
        # rho, rhodot = self.get_risk(T, Tdot, h0, h0dot, rho_min, rho_max)
