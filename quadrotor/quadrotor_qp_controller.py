...
from solve_qp_cvxopt import solve_qp_cvxopt
...
def compute_quadrotor_control(data: dict) -> tuple:
    ...
    data: (dict) --
        ...
        th (np.ndarray) estimated parameter vector
        err (np.ndarray) maximum parameter error vector

    OUTPUTS
    -------
    sol: (np.ndarray) - solution to cascaded quadratic program based controller

    """
    # global F_MODEL, TAU_MODEL

    # Compute nominal control inputs (F, tau1, tau2, tau3)
    thrust_nom,moments_nom = compute_nominal_control(data)

    # Update Force Model with nominal input
    data['unom'] = thrust_nom
    Qt, pt, At, bt, Gt, ht = update_thrust_model(data)

    # Compute safe thrust input
    thrust_data = solve_qp_cvxopt(Qt, pt, At, bt, Gt, ht)
    if not thrust_data['code']:
        return np.zeros(4,), np.zeros(4,),0

    thrust = thrust_data['x'][0]

    # Update Force Model with nominal input
    data['unom'] = moments_nom
    data['thrust'] = thrust
    Qm, pm, Am, bm, Gm, hm = update_moments_model(data)

    # Compute safe moment inputs
    moments_data = solve_qp_cvxopt(Qm, pm, Am, bm, Gm, hm)
    if not moments_data['code']:
        return np.zeros(4, ), np.zeros(4, ), 0

    moments = moments_data['x'][0:3]

    controls = np.concatenate((thrust,moments.flatten()))
    nominal_controls = np.concatenate((np.array([thrust_nom]),moments_nom))

    return controls, nominal_controls, 1
...
def update_thrust_model(data: dict) -> Tuple:
    """ Computes matrices and vectors for solving QP-based safe thrust control law.
    ...
    data: (dict) --
        x:        (np.ndarray) state vector
        unom:     (float) nominal thrust
        err:      (np.ndarray) maximum parameter estimation error
        etaTerms: (np.ndarray) higher order etadot terms for robust-adaptive CBF condition
        Gamma:    (np.ndarray) gain matrix
    ...
    G: 0 -- no equality constraints
    h: 0 -- no equality constraints

    """
    # Unpack data dict
    state      = data['x']
    thrust_nom = data['unom']
    eta        = data['err']
    etaTerms   = data['etaTerms']
    Gamma      = data['Gamma']

    # Objective Function Matrix/Vector
    Q, p = thrust_objective(thrust_nom)
    ...
    # Exponential CBF Condition w/ Class K decision variable: Lf2h + LgLfh*x1 + K1*Lfh + K0*h*x2 >= 0
    As = -np.array([LgLfh,K0*h])
    bs = np.array([Lf2h + K1*Lfh])

    # Input Constraints
    Au = np.array([1, 0], dtype=np.float64)
    Al = np.array([-1, 0], dtype=np.float64)
    bu = np.array([F_MAX])
    bl = np.zeros((1,))
    Ai = np.row_stack((Au, Al))
    bi = np.row_stack((bu, bl))

    # Full set of constraints
    A = np.row_stack((As, Ai))
    b = np.row_stack((bs, bi))
    G = None
    h = None

    return Q,p,A,b,G,h

def update_moments_model(data: dict) -> Tuple:
    """ Computes matrices and vectors for solving QP-based safe moment control law.
    ...
    # Objective Function Matrix/Vector
    Q, p = moments_objective(moments_nom)

    # Relevant Class K function parameters
    rate   = 10.0
    alpha0 = 1.0
    alpha1 = 1.0
    ...
    # Exponential CBF Condition w/ Class K decision variable: Lf2h + LgLfh*x1 + K1*Lfh + K0*h*x2 >= 0
    As = -np.concatenate((LgLfh, np.array([K0*h, K1*Lfh])))
    bs = np.array([Lf2h])

    # Input Constraints based on Thrust
    A1u = np.array([1, 0, 0, 0, 0], dtype=np.float64)
    A1l = np.array([-1, 0, 0, 0, 0], dtype=np.float64)
    A2u = np.array([0, 1, 0, 0, 0], dtype=np.float64)
    A2l = np.array([0, -1, 0, 0, 0], dtype=np.float64)
    Ai  = np.row_stack((A1u,A1l,A2u,A2l))
    bi  = np.row_stack([[thrust * arm_length] for _ in range(4)])
