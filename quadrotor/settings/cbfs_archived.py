...
def cbf(x):
    ...
    return np.array(
        [cbf_altitude(x), cbf_lateral_outer(x), cbf_lateral_inner(x), cbf_velocity_outer(x), cbf_velocity_inner(x),
         cbf_attitude(x)])
...
def cbf2dot_lateral_outer_uncontrolled(x, F):
    q = np.dot(rd_outer(x), Rot(x)[:, 2])
    qdot = np.dot(rddot_outer(x), Rot(x)[:, 2]) + np.dot(rd_outer(x), RotDot(x)[:, 2])
    q2dot = np.dot(rd2dot_outer(x, F), Rot(x)[:, 2]) + np.dot(rd_outer(x), Rot2Dot_uncontrolled(x)[:, 2]) + 2 * np.dot(
        rddot_outer(x), RotDot(x)[:, 2])
...
def cbf2dot_lateral_inner_uncontrolled(x, F):
    q = np.dot(rd_inner(x), Rot(x)[:, 2])
    qdot = np.dot(rddot_inner(x), Rot(x)[:, 2]) + np.dot(rd_inner(x), RotDot(x)[:, 2])
    q2dot = np.dot(rd2dot_inner(x, F), Rot(x)[:, 2]) + np.dot(rd_inner(x), Rot2Dot_uncontrolled(x)[:, 2]) + 2 * np.dot(
        rddot_inner(x), RotDot(x)[:, 2])
...
def cbf2dot_attitude_uncontrolled_0(x):
    return -phi2dot(x)[0] * np.sin(x[6]) * np.cos(x[7]) - the2dot(x)[0] * np.cos(x[6]) * np.sin(x[7]) \
           - (phidot(x) ** 2 + thedot(x) ** 2) * np.cos(x[6]) * np.cos(x[7]) + 2 * phidot(x) * thedot(x) * np.sin(
        x[6]) * np.sin(x[7])
...
def cbf2dot_velocity_outer(x, F):
    return -dist2dot(x, F) - np.dot(vel2dot(x, F), rd_outer(x)) - np.dot(vel(x), rd2dot_outer(x, F)) - 2 * np.dot(
        veldot(x, F), rddot_outer(x))


def cbf3dot_velocity_outer_uncontrolled(x, F):
    return -dist3dot(x, F) - np.dot(vel3dot_uncontrolled(x, F), rd_outer(x)) - np.dot(vel(x),
                                                                                      rd3dot_outer(x, F)) - 3 * np.dot(
        vel2dot(x, F), rddot_outer(x)) - 3 * np.dot(veldot(x, F), rd2dot_outer(x, F))
...
def cbf2dot_velocity_inner(x, F):
    return dist2dot(x, F) - np.dot(vel2dot(x, F), rd_inner(x)) - np.dot(vel(x), rd2dot_inner(x, F)) - 2 * np.dot(
        veldot(x, F), rddot_inner(x))


def cbf3dot_velocity_inner_uncontrolled(x, F):
    return dist3dot(x, F) - np.dot(vel3dot_uncontrolled(x, F), rd_inner(x)) - np.dot(vel(x),
                                                                                     rd3dot_inner(x, F)) - 3 * np.dot(
        vel2dot(x, F), rddot_inner(x)) - 3 * np.dot(veldot(x, F), rd2dot_inner(x, F))
...
def rd3dot_outer(x, F):
    ...
    return np.array([PXo * np.sin(th) * thdot ** 3 - 3 * PXo * np.cos(th) * thdot * th2dot - PXo * np.sin(
        th) * th3dot - x3dot(x, F),
                     -PYo * np.cos(th) * thdot ** 3 - 3 * PYo * np.sin(th) * thdot * th2dot + PYo * np.cos(
                         th) * th3dot - y3dot(x, F),
                     0.])
...
def rd3dot_inner(x, F):
    ...
    return 1 * np.array([PXi * np.sin(th) * thdot ** 3 - 3 * PXi * np.cos(th) * thdot * th2dot - PXi * np.sin(
        th) * th3dot - x3dot(x, F),
                         -PYi * np.cos(th) * thdot ** 3 - 3 * PYi * np.sin(th) * thdot * th2dot + PYi * np.cos(
                             th) * th3dot - y3dot(x, F),
                         0.])
...
def dist3dot(x, F):
    return 2 * (x[0] * xdot(x) + x[1] * ydot(x)) * (
                x[0] * x2dot(x, F) + xdot(x) ** 2 + x[1] * y2dot(x, F) + ydot(x) ** 2) * (x[0] ** 2 + x[1] ** 2) ** (
                       -3 / 2) - \
           3 * (x[0] * xdot(x) + x[1] * ydot(x)) ** 3 * (x[0] ** 2 + x[1] ** 2) ** (-5 / 2) + \
           (x[0] * x2dot(x, F) + xdot(x) ** 2 + x[1] * y2dot(x, F) + ydot(x) ** 2) * (
                       x[0] * xdot(x) + x[1] * ydot(x)) * (x[0] ** 2 + x[1] ** 2) ** (-3 / 2) - \
           (x[0] * x3dot(x, F) + 3 * xdot(x) * x2dot(x, F) + x[1] * y3dot(x, F) + 3 * ydot(x) * y2dot(x, F)) / np.sqrt(
        x[0] ** 2 + x[1] ** 2)
...
def thetadot_outer(x):
    ...
    return ((x[0] - a) * ydot(x) + (b - x[1]) * xdot(x)) / (
                a ** 2 - 2 * a * x[0] + b ** 2 - 2 * b * x[1] + x[0] ** 2 + x[1] ** 2)
...
def theta2dot_outer(x, F):
    ...
    return ((x[0] - a) * y2dot(x, F) + (b - x[1]) * x2dot(x, F)) / (
                a ** 2 - 2 * a * x[0] + b ** 2 - 2 * b * x[1] + x[0] ** 2 + x[1] ** 2) - (
                       ((x[0] - a) * ydot(x) + (b - x[1]) * xdot(x)) * (
                           -2 * a * xdot(x) - 2 * b * ydot(x) + 2 * x[0] * xdot(x) + 2 * x[1] * ydot(x))) / (
                       a ** 2 - 2 * a * x[0] + b ** 2 - 2 * b * x[1] + x[0] ** 2 + x[1] ** 2) ** 2
...
def theta3dot_outer(x, F):
    ...
    return (2 * ((ydot(x)) / (x[0] - a) - ((x[1] - b) * xdot(x)) / (x[0] - a) ** 2) * (
                (2 * (x[1] - b) * ydot(x)) / (x[0] - a) ** 2 - (2 * (x[1] - b) ** 2 * xdot(x)) / (
                    x[0] - a) ** 3) ** 2) / ((x[1] - b) ** 2 / (x[0] - a) ** 2 + 1) ** 3 - (2 * (
                (2 * (x[1] - b) * ydot(x)) / (x[0] - a) ** 2 - (2 * (x[1] - b) ** 2 * xdot(x)) / (x[0] - a) ** 3) * (-(
                (x[1] - b) * x2dot(x, F)) / (x[0] - a) ** 2 + (2 * (x[1] - b) * xdot(x) ** 2) / (x[0] - a) ** 3 - (
                                                                                                                                 2 * xdot(
                                                                                                                             x) * ydot(
                                                                                                                             x)) / (
                                                                                                                                 x[
                                                                                                                         ...
                                                                                                                         y2dot(
                                                                                                                             ...
                                                                                                                             F)) / (
                                                                                                                                 x[
                                                                                                                                     0] - a))) / (
                       (x[1] - b) ** 2 / (x[0] - a) ** 2 + 1) ** 2 - (
                       ((ydot(x)) / (x[0] - a) - ((x[1] - b) * xdot(x)) / (x[0] - a) ** 2) * (
                           -(2 * (x[1] - b) ** 2 * x2dot(x, F)) / (x[0] - a) ** 3 - (
                               8 * (x[1] - b) * xdot(x) * ydot(x)) / (x[0] - a) ** 3 + (
                                       6 * (x[1] - b) ** 2 * xdot(x) ** 2) / (x[0] - a) ** 4 + (
                                       2 * (x[1] - b) * y2dot(x, F)) / (x[0] - a) ** 2 + (2 * ydot(x) ** 2) / (
                                       x[0] - a) ** 2)) / ((x[1] - b) ** 2 / (x[0] - a) ** 2 + 1) ** 2 + (
                       -(x3dot(x, F) * (x[1] - b)) / (x[0] - a) ** 2 - (6 * (x[1] - b) * xdot(x) ** 3) / (
                           x[0] - a) ** 4 + (6 * (x[1] - b) * xdot(x) * x2dot(x, F)) / (x[0] - a) ** 3 - (
                                   3 * x2dot(x, F) * ydot(x)) / (x[0] - a) ** 2 - (3 * xdot(x) * y2dot(x, F)) / (
                                   x[0] - a) ** 2 + (6 * xdot(x) ** 2 * ydot(x)) / (x[0] - a) ** 3 + (y3dot(x, F)) / (
                                   x[0] - a)) / ((x[1] - b) ** 2 / (x[0] - a) ** 2 + 1)
...
def SiLU2dot_uncontrolled(q, qdot, q2dot):
    return KSILU * (q2dot * ((1 + np.exp(-q) + q * np.exp(-q)) / (1 + np.exp(-q)) ** 2) + qdot ** 2 * (
                (np.exp(-q) + np.exp(-2 * q) + 2 * q * np.exp(-2 * q)) / (1 + np.exp(-q)) ** 3))
