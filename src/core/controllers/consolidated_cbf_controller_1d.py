...
class ConsolidatedCbfController(CbfQpController):
    ...
    def formulate_qp(
        ...
        for cc, cbf in enumerate(self.cbfs_individual):
            ...
            Lfh_array[cc] = dhdx_array[cc][:ns] * f(ze) - discretization_error
    ...
    def generate_consolidated_cbf_condition(
        ...
        discretization_error *= np.exp(-1 * H)
    ...
    def augmented_consolidated_cbf(self):
        ...
        return self.consolidated_cbf() * self.adapter.b
...
class AdaptationLaw:
    ...
    def __init__(
        ...
        # Gains and Parameters -- Nonlinear 1D!!
        self.alpha = alpha
        self.wn = 10.0
        self.k_dot_gain = 1.0
    ...
    def k_dot_drift(self, x: NDArray, h: NDArray, Lf: float, Lg: NDArray) -> NDArray:
        ...
        self._k_dot_drift = k_dot_drift[0, 0, 0, :]
        print(f"Kdot Drift: {float(self._k_dot_drift):.2f}")
    ...
    def k_dot_controlled(self, x: NDArray, h: NDArray, Lf: float, Lg: NDArray) -> NDArray:
        ...
        self._k_dot_controlled = k_dot_controlled[0, 0, 0, :]
        print(f"Kdot Contr: {float(self._k_dot_controlled):.2f}")
    ...
    def czero(self, x: NDArray, h: NDArray, Lf: float, Lg: NDArray) -> float:
        ...
        if self.t == 0:
            ...
            count = 0
            while czero < self._delta**2 and count < 10:  # vector @ self.U @ vector.T:
    ...
    @property
    def input_constraint_function(self) -> NDArray:
