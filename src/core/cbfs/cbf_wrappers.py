...
def symbolic_cbf_wrapper_singleagent(cbf_symbolic, *args):
    ...
    def cbf(z: NDArray, is_symbolic: bool = False):
        ...
        if is_symbolic:
            ...
            if (type(ret) is se.Add) or (type(ret) is se.RealDouble) or (type(ret) is se.Mul):
                return float(ret)
            else:
...
def symbolic_cbf_wrapper_multiagent(cbf_symbolic, *args):
    ...
    def cbf(t: float, xe: NDArray, xo: NDArray, is_symbolic: bool = False):
