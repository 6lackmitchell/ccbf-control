...
from .initial_conditions import xg, yg, box_width
...
# Specify files
filepath = pre_path + "Documents/git/ccbf-control/data/bicycle/dynamic/swarm/"
...
txt_list = [
    ax_pos.text(
        x[aa, 0, 0], x[aa, 0, 1], "{}".format(aa + 1), ha="right", va="top", fontsize=24
    )
    for aa in range(nAgents)
]

ax_pos.set(
    ylim=[np.min(y_c) - 1, np.max(y_c) + 1], xlim=[np.min(x_c) - 1, np.max(x_c) + 1]
)
...
ani = animation.FuncAnimation(
    fig=fig_map, func=animate_ego, frames=int(ii / 1), interval=50, repeat=False
)
