...
start_2 = 0.0
ax_pos.plot(
    np.linspace(start_p, end_1, d_points),
    -(LW / 2) * np.ones((d_points,)),
    linewidth=lwidth + 1,
    color="w",
)
ax_pos.plot(
    np.linspace(start_2, end_p, d_points),
    -(LW / 2) * np.ones((d_points,)),
    linewidth=lwidth + 1,
    color="w",
)
ax_pos.plot(
    np.linspace(start_p, end_p, d_points),
    LW + LW / 2 * np.ones((d_points,)),
    linewidth=lwidth + 1,
    color="w",
)
ax_pos.plot(
    np.linspace(start_p, 0, d_points),
    np.linspace(start_p, 0, d_points) * s_th - LW / 2,
    linewidth=lwidth + 1,
    color="w",
)
ax_pos.plot(
    np.linspace(start_p, -LW / (s_th), d_points),
    np.linspace(start_p, -LW / (s_th), d_points) * s_th + LW / 2,
    linewidth=lwidth + 1,
    color="w",
)
...
ax_pos.set(ylim=[-5.0, 6.0], xlim=[-2.0, 50.0])
ax_pos.set(ylim=[-50.0, 6.0], xlim=[-125.0, 50.0])
...
def animate(jj):
    ...
    left_edges = (
        np.array(
            [
                x[np.argmin(x[:, np.max([0, idx - 10]) : idx + 1, 0], 0)[-1], idx, 0]
...
ani = animation.FuncAnimation(
    fig=fig_map, func=animate_ego, frames=int(ii / 1), interval=10, repeat=False
)
