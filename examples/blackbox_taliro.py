...
@blackbox()
def f16_model(
    static: Sequence[float], times: SignalTimes, signals: SignalValues
) -> F16DataT:
    power = 9
    ...
    result = run_f16_sim(
        initial_state, max(times), autopilot, step, extended_states=True
    )
...
options = Options(
    runs=1, iterations=10, interval=(0, 15), static_parameters=initial_conditions
)
