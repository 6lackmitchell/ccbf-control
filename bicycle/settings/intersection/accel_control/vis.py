...
from bicycle.settings_highway_merging import *
...
for aa in range(cbf_val.shape[1]):
    ax_cbfs.plot(t[:ii], cbf_val[:ii, aa, 0], label='h_{}'.format(aa), linewidth=lwidth,
                   color=colors[color_idx[aa, 0]])
    ax_cbfs.plot(t[:ii], cbf_val[:ii, aa, 1], label='h_{}^0'.format(aa), linewidth=lwidth,
                   color=colors[color_idx[aa, 1]], dashes=dash)
...
def animate(jj):
    ...
    end_point = np.max([np.max(x[jj, right_vehicle, 0]) + 2.0, zero_point + 150.0])
